...
public class BaseResidentService implements ResidentService {
  ...
  @Override
    ...
    Pattern cityPattern = compileFilterPattern(filterResident.getCity());
    ...
    for (Resident resident: residentRepository.getResidents()) {
      if (matchesFilter(resident, givenNamePattern, familyNamePattern, streetPattern, cityPattern, calendarFilter)) {
  ...
  private boolean containsWildcards(Resident data) {
    ...
    if ((data.getCity() != null) && (data.getCity().indexOf("*") != -1)) {
      return true;
    }
    return false;
  }
  ...
  private boolean matchesFilter(Resident current, Pattern givenNamePattern, Pattern familyNamePattern, Pattern streetPattern, Pattern cityPattern, Calendar calendarFilter) {
    ...
    if (!matchesAttribute(current.getCity(), cityPattern)) {
      return false;
    }
